...
export const mockMateriaPrimaAdapter = {
  ...
  findAll: async (params: {
    ...
    filteredData.sort((a, b) => {
      const aValue = a[sortBy as keyof MateriaPrima];
      const bValue = b[sortBy as keyof MateriaPrima];
   ...
   */
  findById: async (id: string): Promise<ApiResponse<MateriaPrima>> => {
    await new Promise(resolve => setTimeout(resolve, 50));

    const mockItem: MateriaPrima = {
      id,
      nombre: `Material ${id}`,
   ...
   */
  create: async (data: CreateMateriaPrimaRequest): Promise<ApiResponse<MateriaPrima>> => {
    await new Promise(resolve => setTimeout(resolve, 150));

    const newItem: MateriaPrima = {
      id: `new-${Date.now()}`,
      ...data,
      estatus: "ACTIVO",
      fecha_creacion: new Date(),
   ...
   */
  update: async (id: string, data: UpdateMateriaPrimaRequest): Promise<ApiResponse<MateriaPrima>> => {
    await new Promise(resolve => setTimeout(resolve, 100));

    const updatedItem: MateriaPrima = {
      id,
   ...
   */
  updateEstatus: async (id: string, estatus: 'ACTIVO' | 'INACTIVO'): Promise<ApiResponse<MateriaPrima>> => {
    await new Promise(resolve => setTimeout(resolve, 100));
   ...
   */
  getLowStock: async (params?: { limit?: number }): Promise<ApiResponse<MateriaPrima[]>> => {
    await new Promise(resolve => setTimeout(resolve, 100));

    const lowStockItems: MateriaPrima[] = [
      {
        id: "low-stock-001",
...
export const materiaPrimaAdapter = mockMateriaPrimaAdapter;