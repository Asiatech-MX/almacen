...
export const mockStockAdapter = {
    ...
    if (params.filter) {
      ...
      if (fecha_desde) {
        filteredData = filteredData.filter(item =>
          new Date(item.fecha_movimiento) >= new Date(fecha_desde)
        );
      }

      if (fecha_hasta) {
        filteredData = filteredData.filter(item =>
          new Date(item.fecha_movimiento) <= new Date(fecha_hasta)
        );
      }
    }
   ...
   */
  findById: async (id: string): Promise<ApiResponse<StockMovement>> => {
    await new Promise(resolve => setTimeout(resolve, 50));

    const mockItem: StockMovement = {
      id,
      materia_prima_id: "123e4567-e89b-12d3-a456-426614174000",
      tipo: "ENTRADA",
      cantidad: 10,
   ...
   */
  create: async (data: CreateStockMovementRequest): Promise<ApiResponse<StockMovement>> => {
   ...
   */
  getStockSummary: async (materia_prima_id?: string): Promise<ApiResponse<any>> => {
    await new Promise(resolve => setTimeout(resolve, 100));
   ...
   */
  getLowStock: async (params?: { limit?: number }): Promise<ApiResponse<any[]>> => {
    await new Promise(resolve => setTimeout(resolve, 100));
   ...
   */
  getStockStats: async (params?: { materia_prima_id?: string; periodo_dias?: number }): Promise<ApiResponse<any>> => {
    await new Promise(resolve => setTimeout(resolve, 100));
   ...
   */
  getMovementHistory: async (materia_prima_id: string, params?: {
    limit?: number;
    periodo_dias?: number;
...
export const stockAdapter = mockStockAdapter;