...
const scrollerVariants = cva("", {
  variants: {
    orientation: {
      vertical: [
        "overflow-y-auto",
        "overflow-x-hidden", // Prevenir overflow horizontal no deseado
      ...
      horizontal: [
        "overflow-x-auto",
        "overflow-y-hidden", // Prevenir overflow vertical no deseado
...
function Scroller(props: ScrollerProps) {
  ...
  React.useLayoutEffect(() => {
    ...
    // For viewportAware functionality
    if (viewportAware) {
      const checkScrollability = () => {
        ...
        // Add/remove classes based on scrollability
        if (needsScroll) {
  ...
  }, [orientation, offset, withNavigation, viewportAware]);
  ...
  const ScrollerImpl = (
    <ScrollerPrimitive
      ...
      className={cn(
        ...
        viewportAware && "transition-all duration-200",
        className,
      )}
    />
  );
...
const scrollButtonVariants = cva(
  "absolute z-10 transition-opacity [&>svg]:size-4 [&>svg]:opacity-80 hover:[&>svg]:opacity-100 bg-background/80 backdrop-blur-sm border rounded-md shadow-sm",
  {
    variants: {
      direction: {
